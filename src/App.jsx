...
function App() {
  ...
  useEffect(() => {
    fetch('https://uninterested-rose-tadpole.cyclic.app/pets_pricen')
    .then(res => res.json())
    .then(result => {
      console.log(result)
      setPetPrice(result)
    ...
    fetch('https://uninterested-rose-tadpole.cyclic.app/pets_price_chart')
    .then(res => res.json())
    .then(result => {
      console.log(result)
      setPetChart({
  ...
  return (
    <div>
      <h1>ปุรเชษฐ วิลัยกรวด 6402621</h1>
      <ul>
        {petPrices.map(pet => (
          <li key={pet.id}>
